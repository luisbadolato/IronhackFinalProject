...
@Component({
  selector: 'app-chords-module',
  templateUrl: './chords-module.component.html',
  styleUrls: ['./chords-module.component.scss']
})
export class ChordsModuleComponent implements OnInit {
  ...
  loadSubjects(): void {
    this.appService.getChordsSubjects().subscribe(
      data => {
  ...
  loadLesson(id: number) {
    this.appService.getChordsLesson(id).subscribe(
      data => {
  ...
  loadUserLessonData(id: number, moduleName: string) {
    if (this.tokenService.getUserName()) {
      this.userName = this.tokenService.getUserName()!;
      this.appService.getDiscreetLessonData(this.userName, id, "CHORDS_MODULE").subscribe(
        data => {
          try {
