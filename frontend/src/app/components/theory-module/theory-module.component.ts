...
@Component({
  selector: 'app-theory-module',
  templateUrl: './theory-module.component.html',
  styleUrls: ['./theory-module.component.scss']
})
export class TheoryModuleComponent implements OnInit {
  ...
  loadSubjects(): void {
    this.appService.getTheorySubjects().subscribe(
      data => {
  ...
  loadLesson(id: number) {
    this.appService.getTheoryLesson(id).subscribe(
      data => {
  ...
  loadUserLessonData(id: number, moduleName: string) {
    if (this.tokenService.getUserName()) {
      this.userName = this.tokenService.getUserName()!;
      this.appService.getDiscreetLessonData(this.userName, id, "THEORY_MODULE").subscribe(
        data => {
          try {
