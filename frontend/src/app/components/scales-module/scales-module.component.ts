...
@Component({
  selector: 'app-scales-module',
  templateUrl: './scales-module.component.html',
  styleUrls: ['./scales-module.component.scss']
})
export class ScalesModuleComponent implements OnInit {
  ...
  loadSubjects(): void {
    this.appService.getScaleSubjects().subscribe(
      data => {
  ...
  loadUserData(): void {
    if (this.tokenService.getUserName()) {
      ...
      // this.loadMastery(this.userName, "SCALES_MODULE");
    }
  }
  ...
  loadLesson(id: number) {
    this.appService.getScaleLesson(id).subscribe(
      data => {
